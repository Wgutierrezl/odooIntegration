...
export default function EmployeesPage() {
  ...
  const [form, setForm] = useState({ name: '', work_email: '', job_title: '' });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowForm(false);
      setForm({ name: '', work_email: '', job_title: '' });
    },
  });
      ...
      {showForm && (
        <div className="bg-white p-4 rounded-xl shadow-sm border mb-4">
          ...
          <form onSubmit={(e) => { e.preventDefault(); createMutation.mutate(form); }} className="grid grid-cols-3 gap-3">
            ...
            <input placeholder="Email" value={form.work_email} onChange={(e) => setForm({ ...form, work_email: e.target.value })} className="px-3 py-2 border rounded-lg outline-none" />
            <input placeholder="Job Title" value={form.job_title} onChange={(e) => setForm({ ...form, job_title: e.target.value })} className="px-3 py-2 border rounded-lg outline-none" />
            ...
            <tbody>
              {data?.items?.map((e: any) => (
                <tr key={e.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-500">{e.work_email || '—'}</td>
                  <td className="px-4 py-3 text-gray-500">{e.job_title || '—'}</td>
                  <td className="px-4 py-3 text-gray-500">{e.department_id?.[1] || '—'}</td>
                </tr>
              ))}
              {!data?.items?.length && (
