  ...
  return (
    <div>
      ...
      <div className="relative mb-4">
        ...
        <input type="text" placeholder="Buscar clientes..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
      </div>
      ...
      {showForm && (
        <div className="bg-white p-4 rounded-xl shadow-sm border mb-4">
          ...
          <form onSubmit={(e) => { e.preventDefault(); createMutation.mutate(form); }} className="grid grid-cols-3 gap-3">
            ...
            <input placeholder="Correo" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="px-3 py-2 border rounded-lg outline-none" />
            <input placeholder="Teléfono" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="px-3 py-2 border rounded-lg outline-none" />
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr className="text-left text-gray-500">
                ...
                <th className="px-4 py-3">NIT/VAT</th>
            ...
            <tbody>
              {data?.items?.map((c: any) => (
                <tr key={c.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-500">{c.street || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
